...
function playbrick_get_bricks_global_variables() {
	$variables = get_option( 'bricks_global_variables', [] );

	return is_array( $variables ) ? $variables : [];
}

function playbrick_register_bricks_theme_token( array &$tokens, $token, $value ) {
	$token = trim( (string) $token );
	$value = trim( (string) $value );

	if ( $token === '' || $value === '' ) return;
	if ( preg_match( '/[^A-Za-z0-9_-]/', $token ) || preg_match( '/^[-]/', $token ) ) return;
	if ( preg_match( '/[;{}]/', $value ) ) return;

	$base_token = $token;
	$suffix     = 2;

	while ( isset( $tokens[ $token ] ) && $tokens[ $token ] !== $value ) {
		$token = $base_token . '-' . $suffix;
		$suffix++;
	}
...
function playbrick_bricks_global_variable_css_var_name( array $variable ) {
	$name  = isset( $variable['name'] ) && is_string( $variable['name'] ) ? trim( $variable['name'] ) : '';
	$value = isset( $variable['value'] ) && is_scalar( $variable['value'] ) ? trim( (string) $variable['value'] ) : '';

	if ( $name === '' || $value === '' ) return '';
	if ( preg_match( '/[^A-Za-z0-9_-]/', $name ) || preg_match( '/^[-]/', $name ) ) return '';

	return '--' . $name;
}

function playbrick_bricks_global_variable_css_value( array $variable ) {
	$value = isset( $variable['value'] ) && is_scalar( $variable['value'] ) ? trim( (string) $variable['value'] ) : '';

	if ( $value === '' || preg_match( '/[;{}]/', $value ) ) return '';

	return $value;
}

function playbrick_bricks_global_variable_theme_token( array $variable ) {
	$name = isset( $variable['name'] ) && is_string( $variable['name'] ) ? trim( $variable['name'] ) : '';
...
function playbrick_collect_bricks_global_variable_theme_tokens( array $variables ) {
	...
	foreach ( $variables as $variable ) {
		...
		$token   = playbrick_bricks_global_variable_theme_token( $variable );
		$css_var = playbrick_bricks_global_variable_css_var_name( $variable );
		$value   = playbrick_bricks_global_variable_css_value( $variable );
...
function playbrick_collect_bricks_theme_tokens( array $palette, array $global_variables = [] ) {
	...
	foreach ( playbrick_collect_bricks_global_variable_theme_tokens( $global_variables ) as $token => $css_var ) {
		playbrick_register_bricks_theme_token( $tokens, $token, $css_var );
	}

	return $tokens;
}

function playbrick_build_bricks_theme_css( array $palette, array $global_variables = [] ) {
	$css = "/* Auto-generated by PlayBrick. Do not edit — regenerated from Bricks Style Manager variables / palette. */\n";

	$tokens = playbrick_collect_bricks_theme_tokens( $palette, $global_variables );

	if ( ! $tokens ) {
		return $css . "/* No Bricks Style Manager variables or color palette found. Save variables in Bricks Style Manager or a color palette, then rebuild. */\n";
	}
...
function playbrick_write_bricks_tailwind_source( $playground_path, array $classes, $custom_css = '', $theme_css = '' ) {
	...
	if ( file_put_contents( $path, $html, LOCK_EX ) === false ) return false;
	if ( file_put_contents( $raw_path, $raw, LOCK_EX ) === false ) return false;
	if ( file_put_contents( $custom_css_path, $custom_css, LOCK_EX ) === false ) return false;

	if ( $theme_css !== '' ) {
		if ( file_put_contents( $theme_css_path, $theme_css . "\n", LOCK_EX ) === false ) return false;
	}
